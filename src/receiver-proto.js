...
// metrics (DIS)
const metricsFile = fs.createWriteStream(`${config.PROTO_RECEIVER_METRICS_DIR}/proto-receiver-metrics-${execNum}.csv`)
metricsFile.write('Msg_Received,Received_Time,Decoding_Time\n')
let metrics = []

// message data (DIS)
const messagesFile = fs.createWriteStream(`${config.PROTO_MESSAGES_DIR}/proto-received-msg-${execNum}.csv`)
messagesFile.write('Force,ID,Kind,Domain,Country,Category,Timestamp,Location_X,Location_Y,Location_Z\n')
let messages = []

// server functions
server.on('listening', () => {
  console.log(`[PROTO-RECEIVER] => server listening at ${config.UDP_RECEIVER_IP}:${config.UDP_PORT}...`)
})
